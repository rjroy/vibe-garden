...
related:
  - .lore/work/brainstorm/lore-directory-redesign.md
  - .lore/work/brainstorm/principles-for-capture-skills.md
  - .lore/work/brainstorm/distill-function.md
  - .lore/work/brainstorm/learn-dialog.md
  - .lore/work/issues/roadmap-lore-redesign.md
  - .lore/work/issues/design-learned-structure.md
  - .lore/work/issues/design-extraction-skill.md
req-prefix: REDESIGN
---
...
Restructure the `.lore/` tree into three directories (`work/`, `reference/`, `learned/`), rework the capture skills (`/retro` strips down, `/excavate` becomes `/distill`, `/learn` is new), migrate every existing skill and agent that writes or reads `.lore/` paths, consolidate the frontmatter schema, and give `/tend` a migration mode that moves existing projects onto the new structure.
...
**In scope:**
- Three-directory model (`.lore/work/`, `.lore/reference/`, `.lore/learned/`).
...
- `/excavate` rename to `/distill` with two seed modes (`code`, `work`) and a tightened shape rule for reference.
...
- Agent description updates for `lore-researcher`, `spec-reviewer`, `design-reviewer`, `plan-reviewer`, `fresh-lore`, and Celeste's reference to `.lore/reference/vision.md`.
...
- Per-diagram splitting decisions (which individual diagrams belong in work vs reference). Per-file judgment call; no design needed.
...
Directly follows `.lore/work/brainstorm/lore-directory-redesign.md`.

- **`.lore/work/`** — work scaffolding. Session-bound. Dissolves or archives after the work is done.
...
Two one-directional pipelines: work → reference (via `/distill`) and work → learned (via `/learn`). Success does not graduate.

## Requirements

### Directory structure

- REQ-REDESIGN-1: `.lore/work/`, `.lore/reference/`, and `.lore/learned/` are the three canonical top-level directories under `.lore/` owned by the lore-development plugin.
...
- REQ-REDESIGN-3: `vision.md` moves from `.lore/reference/vision.md` to `.lore/reference/vision.md`. The `/vision` skill and Celeste's agent description are updated to the new path.
- REQ-REDESIGN-4: `.lore/learned/` is created on first `/learn` invocation in a project. It does not need to pre-exist.
- REQ-REDESIGN-5: Diagrams are split by purpose (session-bound → `work/diagrams/`, current-state → `reference/diagrams/`). `/ddp` asks which on write; per-diagram judgment. Default to `work/diagrams/` when ambiguous.
...
|-------|----------|----------|
| `/brainstorm` | `.lore/work/brainstorm/` | `.lore/work/brainstorm/` |
| `/specify` | `.lore/work/specs/` | `.lore/work/specs/` |
| `/design` | `.lore/work/design/` | `.lore/work/design/` |
| `/prep-plan` | `.lore/work/plans/` | `.lore/work/plans/` |
| `/plan-breakdown` | `.lore/work/tasks/` | `.lore/work/tasks/` |
| `/implement` | `.lore/work/notes/` | `.lore/work/notes/` |
| `/simplify` | `.lore/work/notes/` | `.lore/work/notes/` |
| `/research` | `.lore/work/research/` | `.lore/work/research/` |
| `/retro` (reshaped) | `.lore/work/retros/` | `.lore/work/retros/` |
| `/excavate` → `/distill` | `.lore/reference/` (writes), `.lore/work/excavations/` (index) | `.lore/reference/` (unchanged for feature docs), `.lore/work/excavations/` (index) |
| `/file-issue` | `.lore/work/issues/` | `.lore/work/issues/` |
| `/review-ideas` | reads `.lore/work/ideas/`, writes `.lore/work/issues/` | reads `.lore/work/ideas/`, writes `.lore/work/issues/` |
| `/define-validation` | `.lore/work/validation/` | `.lore/work/validation/` |
| `/update-stubs` | reads `.lore/work/specs/`, writes `.lore/work/stubs/index.md` | reads `.lore/work/specs/`, writes `.lore/work/stubs/index.md` |
| `/back-propagate` | reads `.lore/work/plans/`, `.lore/work/notes/`, `.lore/work/retros/`, writes `.lore/work/specs/` | all under `.lore/work/` |
| `/ddp` | `.lore/work/diagrams/` | split: `.lore/work/diagrams/` or `.lore/reference/diagrams/` (see REQ-REDESIGN-5) |
| `/vision` | `.lore/reference/vision.md` | `.lore/reference/vision.md` |
| `/poke-holes` | reads only; no write path | update any path references in prompts |
| `/learn` (new) | — | `.lore/learned/` |
| idea-capture hook | `.lore/work/ideas/` | `.lore/work/ideas/` |

- REQ-REDESIGN-7: The raw idea queue (`.lore/work/ideas/` under the old scheme; `.lore/work/ideas/` under the new) continues to hold non-frontmatter markdown per the existing `/review-ideas` convention. `/tend` does not flag missing frontmatter for files under this path.
...
- REQ-REDESIGN-9: Status values collapse into three sets, one per top-level directory:
  - **Work documents** retain the meaningful per-type statuses from the current schema, since each work artifact type still has a lifecycle:
...
- REQ-REDESIGN-11: Notes-specific `source` field and task-specific `source` + `sequence` fields persist unchanged; path values inside them migrate to the new tree (e.g., `source: .lore/work/plans/auth-flow.md`).
...
- REQ-REDESIGN-16: The `/retro` prompt directs the skill to describe what happened rather than interpret it. The vocabulary of analysis (`lesson`, `insight`, `we learned`, `takeaway`) is forbidden in capture output. See `.lore/work/brainstorm/principles-for-capture-skills.md` for the binding rules.
...
- REQ-REDESIGN-19: `/tend migrate` detects legacy structure by presence of any of the old top-level directories (`.lore/work/brainstorm/`, `.lore/work/specs/`, `.lore/work/design/`, `.lore/work/plans/`, `.lore/work/tasks/`, `.lore/work/notes/`, `.lore/work/research/`, `.lore/work/retros/`, `.lore/work/issues/`, `.lore/work/ideas/`, `.lore/work/validation/`, `.lore/work/stubs/`, `.lore/work/excavations/`, `.lore/work/diagrams/`) or `.lore/reference/vision.md`.
- REQ-REDESIGN-20: `/tend migrate` moves files to their new locations per REQ-REDESIGN-6, creates the `work/`, `reference/`, `learned/` directories as needed, and updates every internal path reference (`related:`, `source:`, in-body markdown links) to the new paths.
- REQ-REDESIGN-21: For diagrams, `/tend migrate` defaults all existing `.lore/work/diagrams/*` files to `.lore/work/diagrams/`. The user can manually promote individual diagrams to `.lore/reference/diagrams/` afterward; `/tend` does not decide per-diagram during migration.
...
- REQ-REDESIGN-25: `/tend`'s non-migrate modes (`status`, `tags`, `filenames`, `directories`) are updated to treat `work/`, `reference/`, `learned/` as the standard directories. Legacy top-level directories (if present) are flagged as orphans with a pointer to `/tend migrate`.

### `/excavate` → `/distill` rewrite (roadmap step 5)

Directs the refactor. Reasoning is in `.lore/work/brainstorm/distill-function.md`; do not re-derive here.

- REQ-REDESIGN-26: `/excavate` is renamed to `/distill`. The plugin command path changes; skill files move accordingly.
- REQ-REDESIGN-27: `/distill` supports two seed modes: `/distill code` (reshape of existing excavate behavior) and `/distill work` (new mode that reads specs, plans, and brainstorms as seed material). Both modes run the same code-verifying core.
...
- REQ-REDESIGN-32: The excavation index (currently `.lore/work/excavations/index.md`) moves to `.lore/work/excavations/index.md` and continues to track documented features, discovered-but-undocumented features, and unexplored entry points.
...
Directs the build. Reasoning is in `.lore/work/brainstorm/learn-dialog.md`; do not re-derive here.
...
- REQ-REDESIGN-37: Entries enforce the asymmetric shape at the artifact level: "don't do X because Y happened" or "if you find yourself doing X, stop — here's why." A `/learn` entry that reads like "do X because it worked" is malformed. See `.lore/work/brainstorm/principles-for-capture-skills.md` principle 2.
...
- REQ-REDESIGN-42: `lore-researcher`'s description and search-path lists update to search the three new directories. Retros, specs, brainstorms, research, and issues all live under `work/`; feature docs under `reference/`; lessons under `learned/`. Search priority order is rewritten to prioritize `learned/` highest (operational imperatives), then `reference/` (solidified knowledge), then `work/` (session material) — an inversion of current priority, which leads with retros.
- REQ-REDESIGN-43: `spec-reviewer`, `design-reviewer`, and `plan-reviewer` agent descriptions update paths: `.lore/work/specs/` → `.lore/work/specs/`, `.lore/work/design/` → `.lore/work/design/`, `.lore/work/plans/` → `.lore/work/plans/`. Fallback-review-save path (`.lore/reviews/`) moves to `.lore/work/reviews/`.
- REQ-REDESIGN-44: `fresh-lore` agent description updates path examples to the new tree.
- REQ-REDESIGN-45: Celeste's agent description (guild-hall worker, external to lore-development plugin) updates to reference `.lore/reference/vision.md` instead of `.lore/reference/vision.md`. This is the only cross-plugin touchpoint the spec requires; all other guild-hall surfaces stay untouched.
...
- [ ] All three directories (`work/`, `reference/`, `learned/`) are the only `.lore/` subdirectories touched by lore-development skills on new projects.
...
- [ ] `/distill code` and `/distill work` both work. A `/distill work` session on a spec produces either a reference update, a mismatch surfaced for user resolution, or zero files — never a fabricated promotion candidate.
...
- **Path-string audit.** After the refactor, grep the `lore-development/` tree for `.lore/work/brainstorm/`, `.lore/work/specs/`, `.lore/work/design/`, `.lore/work/plans/`, `.lore/work/tasks/`, `.lore/work/notes/`, `.lore/work/research/`, `.lore/work/retros/`, `.lore/work/issues/`, `.lore/work/ideas/`, `.lore/work/validation/`, `.lore/work/stubs/`, `.lore/work/excavations/`, `.lore/work/diagrams/`, and `.lore/reference/vision.md` (exact strings with leading `.lore/`, not nested under `work/`). Any hit in a SKILL.md, agent description, or shared reference file must be justified (e.g., migration documentation that intentionally shows the old path) or fixed.
...
- The three-directory model is not negotiable within this spec. Debate about the boundary between work and reference, or about whether learned should exist, belongs in a new brainstorm, not a requirement revision.
...
- Guild-hall–owned paths (`.lore/commissions/`, `.lore/meetings/`, `.lore/heartbeat.md`, `.lore/lore-agents.md`) are not moved under `work/`. They are separate concerns.
...
**1. `/learn` as extractor vs dialog.** `.lore/work/brainstorm/lore-directory-redesign.md` describes extraction as an AI-scans-work-for-candidates skill fired at the front of new work. `.lore/work/brainstorm/learn-dialog.md` explicitly supersedes that: `/learn` is user-invoked only, doesn't scan, doesn't propose, doesn't fire at the front of new work. The roadmap names the supersession. The spec follows the learn-dialog framing (REQ-REDESIGN-34 through REQ-REDESIGN-41). `.lore/work/issues/design-extraction-skill.md`'s automated-scan framing is formally retired by this spec.

**2. The "named promotion verb" open question.** `.lore/work/brainstorm/lore-directory-redesign.md` flagged that work-to-reference promotion had no named operation. `.lore/work/brainstorm/distill-function.md` resolves this by renaming `/excavate` to `/distill` and adding `/distill work` as the second seed mode. The spec adopts the resolved form (REQ-REDESIGN-26 through REQ-REDESIGN-33). The lore-directory-redesign brainstorm's open question on this point is closed by the distill brainstorm.

## Context

Binding sources (read these before acting on any requirement):

- `.lore/work/brainstorm/lore-directory-redesign.md` — three-directory model.
- `.lore/work/brainstorm/principles-for-capture-skills.md` — binding rules for `/retro` and `/learn` (no N-demanding templates; mistakes only; observation separate from interpretation).
- `.lore/work/brainstorm/distill-function.md` — shape of `/distill`, the code-verifying core, the reference shape rule.
- `.lore/work/brainstorm/learn-dialog.md` — shape of `/learn`, two-path opening, write discipline.
- `.lore/work/issues/roadmap-lore-redesign.md` — step sequencing and ordering constraints.
- `.lore/work/issues/design-learned-structure.md` — open design work on the internal structure of `learned/`. The spec defers to this issue for anything beyond minimum frontmatter.
- `.lore/work/issues/design-extraction-skill.md` — superseded by `.lore/work/brainstorm/learn-dialog.md`; preserved for historical context only.
