...
- The input must be a plan artifact (`.lore/work/plans/*.md`)
...
- REQ-PBD-6: Task files are saved to `.lore/work/tasks/<plan-name>/NNN-<task-name>.md` where NNN is a zero-padded sequence number (001, 002, ...) and task-name is a kebab-case description.
- REQ-PBD-7: The plan-name directory matches the plan's filename without extension. If the plan is `.lore/work/plans/auth-flow.md`, tasks go in `.lore/work/tasks/auth-flow/`.

### Implement Integration

- REQ-PBD-8: When implement is invoked with a plan, it checks for a corresponding `.lore/work/tasks/<plan-name>/` directory. If task files exist, implement uses them as its phase list instead of deriving phases from the plan's steps.
...
|------|---------------|--------|
| Tasks saved | Decomposition complete, user approves | `.lore/work/tasks/<plan-name>/` directory with task files |
...
- [ ] Skill reads a plan and produces task files in `.lore/work/tasks/`
...
- Implementation skill spec: `.lore/work/specs/lore-development/implementation-skill.md`
- Retro (why old breakdown was removed): `.lore/work/retros/lore-development/remove-breakdown-execute.md`
