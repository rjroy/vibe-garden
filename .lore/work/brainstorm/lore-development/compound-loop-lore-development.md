...
related: [.lore/work/specs/lore-development/lore-researcher-agent.md]
---
...
2. **Store** - Put it somewhere findable with queryable structure (`.lore/work/retros/` with frontmatter)
...
**Proposed**: `lore-researcher` agent spawned by `/specify` and `/prep-plan` to grep `.lore/work/retros/` for related work before the human starts writing.
...
2. Grep `.lore/work/retros/` frontmatter for matches
3. Optionally scan `.lore/work/specs/` for related prior work
...
- Should the agent also search `.lore/work/brainstorm/` and `.lore/work/specs/` for related context?
