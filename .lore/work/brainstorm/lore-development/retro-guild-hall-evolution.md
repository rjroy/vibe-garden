...
related:
  - .lore/work/brainstorm/lore-development/lessons-graduation.md
  - .lore/work/brainstorm/lore-development/compound-loop-lore-development.md
  - .lore/work/retros/lore-development/compound-loop-implementation.md
---
...
1. Review `.lore/work/specs/` and `.lore/work/plans/` from the current session
2. Reflect on what happened vs. what was expected (in conversation)
3. Write a retro to `.lore/work/retros/`
...
**Could a retro commission read commission logs and produce a retro document?** Yes, technically. A retro commission could be given a list of commission file paths, read them, synthesize across them, and produce a `.lore/work/retros/` document. This is pure file I/O + summarization. No new tooling needed.
...
A `.lore/work/retros/` document structured differently from the current template. The current template is feature-focused: "What went well with X feature?" A Guild Hall retro is process-focused: "What patterns emerged across these commissions?"
...
**Where the output lives**: Still `.lore/work/retros/`, but perhaps with a subdirectory convention. The current `.lore/work/retros/lore-development/` pattern is by module. A Guild Hall retro might use a `guild-hall/` subdirectory or a `commissions/` subdirectory to distinguish it from feature-level retros.

## Retro as a Scheduled Commission

This is the most interesting design option and the one with the most unknowns.

**What it would look like**: A commission dispatched weekly (or on some cadence) to a retro-focused worker. The commission prompt is auto-generated from the commission log since the last retro. The retro worker synthesizes, produces a draft retro, and submits it. The draft goes into `.lore/work/retros/` with status `draft`. A subsequent human interaction reviews and graduates lessons.
...
6. **How does the retro surface to the Guild Master?** A retro commission that produces a file and submits a result is invisible unless the Guild Master checks `.lore/work/retros/`. Notify hook? A meeting summary that mentions the retro artifact? Something needs to close the loop.
