...
related: [.lore/work/brainstorm/lore-development/document-lifecycle-and-lore-hygiene.md]
---
...
3. Creates or appends to `.lore/work/ideas/2026-02-18.md`
4. Done. The user keeps working.

Produces `.lore/work/ideas/2026-02-18.md`:
...
3. Save a structured issue to `.lore/work/issues/`.
...
### `.lore/work/issues/` - Lightweight actionable items
...
|--------|-----------|----------|------------|
| Ideas | Bottom-up (observed) | `.lore/work/ideas/` | `/idea` |
| Issues | Bottom-up (refined) | `.lore/work/issues/` | `/review-ideas` |
| Tasks | Top-down (from plans) | `.lore/work/tasks/` | `/plan-breakdown` |
...
Compass Rose serves strategic work: what should we build next, backlog health, priority across a body of work. `.lore/work/issues/` serves tactical work: this is broken, fix it now or soon. If something needs team visibility, `/review-ideas` could offer to graduate it to GitHub via Compass Rose. The local issue becomes the research backing the GitHub issue.
...
- The `.lore/work/ideas/` and `.lore/work/issues/` directory conventions
...
2. If yes: extract text after prefix, append bullet to `.lore/work/ideas/YYYY-MM-DD.md`, return block decision with confirmation
...
2. **What about triage across issues?** If `.lore/work/issues/` accumulates, you need a way to prioritize. Possibly a `/triage-issues` skill that ranks by what matters most. Or maybe that's where Compass Rose re-enters, operating on local issues instead of GitHub.
...
- Define `.lore/work/issues/` document structure formally
