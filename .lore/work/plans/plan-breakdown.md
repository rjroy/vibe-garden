...
related:
  - .lore/work/specs/lore-development/plan-breakdown.md
...
**Spec**: `.lore/work/specs/lore-development/plan-breakdown.md`
...
- REQ-PBD-6: Storage path `.lore/work/tasks/<plan-name>/NNN-<task-name>.md` → Step 2
...
Add a `task` row to the "Status Values by Document Type" table with directory `.lore/work/tasks/` and statuses `pending`, `complete`, `skipped`.
...
**Input**: Path to a plan artifact (`.lore/work/plans/*.md`). Read the plan. If the plan has a spec reference, load the spec too (for requirement IDs in the Why section).
...
**Task file output** (REQ-PBD-4, 5, 6, 7): Include the task file template inline (frontmatter with source/sequence/status, then What/Validation/Why/Files sections). Save to `.lore/work/tasks/<plan-name>/NNN-<task-name>.md`. Reference the frontmatter schema for field definitions.
...
When the input is a plan, check whether `.lore/work/tasks/<plan-name>/` exists (where `<plan-name>` matches the plan's filename without extension). If the directory exists and contains task files:
...
Use the Task tool with `subagent_type: general-purpose`. Prompt the agent to read the spec at `.lore/work/specs/lore-development/plan-breakdown.md` and review all three modified/created files:
