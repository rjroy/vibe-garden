...
related:
  - .lore/work/specs/lore-redesign.md
...
**Spec**: `.lore/work/specs/lore-redesign.md`
...
- **Scripts** under `lore-development/scripts/`: `frontmatter_schema.py` (machine-readable schema used by `validate_frontmatter.py`), `validate_frontmatter.py` (consumed by `/tend status`), `idea_hook.py` (writes to `.lore/work/ideas/`), plus a `tests/` dir.
...
**Cross-plugin touchpoint**: Celeste lives in guild-hall (outside `lore-development/`). Her description references `.lore/reference/vision.md` and needs to move to `.lore/reference/vision.md` (REQ-REDESIGN-45). Spec constraint: this is the only cross-plugin change required.
...
**Already-migrated exemplar**: `lore-development/skills/excavate/SKILL.md` already has `artifact_path: .lore/reference` in frontmatter — reference is the one pre-existing directory that stays put. Everything else moves under `work/`.

**Pre-migration state of this repo's `.lore/`**: old-layout directories present at `.lore/{brainstorm,specs,plans,design,research,retros,issues,commissions,ideas,diagrams,excavations,tasks,notes,stubs,validation}/` plus `.lore/reference/vision.md`. This repo is the canonical end-to-end fixture for `/tend migrate`.
...
- `lore-development/scripts/tests/fixtures/` (reorganize into `fixtures/work/`, `fixtures/reference/`, `fixtures/learned/` subtrees)
...
2. Update the "Additional Status Values by Document Type" table to map to `work/`, `reference/`, `learned/` subdirectories.
...
5. Regenerate `frontmatter_schema.py` to match: per-directory status sets, path constants pointing at `.lore/work/*`, `.lore/reference/*`, `.lore/learned/*`. Keep the comment trail back to the schema document.
6. Update `lore-development/scripts/tests/test_frontmatter_schema.py`. The existing `SCHEMA_DOCUMENT_TYPES` list hardcodes old-layout keys (`brainstorm`, `specs`, `design`, `retros`, `research`, `diagrams`, `plans`, `notes`, `tasks`, `reference`, `issues`) and will fail `test_every_document_type_has_entry` as soon as `frontmatter_schema.py` is rewritten. Update this list to the new keyset (`work/brainstorm`, `work/specs`, ..., `reference`, `learned`) or whatever keying convention the rewritten schema adopts. Also reorganize `lore-development/scripts/tests/fixtures/` into `fixtures/work/`, `fixtures/reference/`, `fixtures/learned/` subtrees so the fixture-tree validation step below can run.
7. Update `validate_frontmatter.py` path expectations if it hardcodes directory names beyond `frontmatter_schema.py`.
8. Rewrite `tend/references/directories.md` to enumerate the new standard directories (`.lore/work/*`, `.lore/reference/*`, `.lore/learned/*`). Flag legacy top-levels (old 13) as orphans with a pointer to `/tend migrate`. Update the "note on `.lore/work/ideas/`" to `.lore/work/ideas/`.
...
- `lore-development/scripts/idea_hook.py` — writes to `.lore/work/ideas/`
...
- Output paths in prose (`Save to .lore/X/` → `Save to .lore/work/X/`).
...
Idea hook: change the write path from `.lore/work/ideas/` to `.lore/work/ideas/`. Update hook docstrings.
...
REQ-REDESIGN-7: `/review-ideas` skill's handling of `.lore/work/ideas/` as frontmatter-free queue is unchanged. Its prose paths get updated; its semantics do not.

REQ-REDESIGN-5: `/ddp` gains the split-by-purpose dialog (work vs reference diagrams, default to work when ambiguous). This is a behavior addition, small enough to land here. If the change grows beyond ~30 lines, split into its own step.
...
1. `lore-researcher.md`: rewrite search paths to `.lore/work/{brainstorm,specs,design,plans,notes,research,retros,issues,ideas,tasks,validation,stubs,excavations}/`, `.lore/reference/*`, `.lore/learned/*`. Invert search priority per REQ-REDESIGN-42: `learned/` first (operational imperatives), `reference/` second (solidified knowledge), `work/` third (session material). This reverses the current retro-first bias.
2. `spec-reviewer.md`: path update `.lore/work/specs/` → `.lore/work/specs/`. Fallback-review-save `.lore/reviews/` → `.lore/work/reviews/`.
3. `design-reviewer.md`: path update `.lore/work/design/` → `.lore/work/design/`. Same fallback rename.
4. `plan-reviewer.md`: path update `.lore/work/plans/` → `.lore/work/plans/`. Same fallback rename.
...
1. Write `tend_migrate.py`:
   - Detect legacy structure by scanning for any of the 14 old top-level directories + `.lore/reference/vision.md` (REQ-REDESIGN-19).
   - Build move plan per REQ-REDESIGN-20 (uses the migration table from REQ-REDESIGN-6).
   - Diagrams default to `work/diagrams/` (REQ-REDESIGN-21).
...
2. Unit tests:
   ...
   - **Fenced-code-block preservation**: add a fixture document whose body contains a fenced code block referencing an old path (e.g., a `bash` block that cat-es `.lore/work/brainstorm/foo.md`). Assert the script leaves code-block content as-is — these are documentation, not live links.
...
- Run `/tend status`, `/tend tags`, `/tend filenames`, `/tend directories` modes against a post-migration fixture tree. Confirm they treat `work/`, `reference/`, `learned/` as standard and flag no false orphans.
...
2. Rewrite SKILL.md body per REQ-REDESIGN-26 – 32:
   - Two seed modes: `/distill code` (reshape of current excavate behavior — tuned down per REQ-REDESIGN-29's shape rule) and `/distill work` (new — reads from `.lore/work/specs/`, `.lore/work/plans/`, `.lore/work/brainstorm/` as seed material).
   ...
   - Excavation index moves to `.lore/work/excavations/index.md` (REQ-REDESIGN-32). Reference docs themselves stay in `.lore/reference/` (unchanged).
...
- Manual trigger: `/distill work .lore/work/specs/<test-spec>.md` → skill loads, asks user for seed confirmation, does not pre-scan.

**Review**:
- **plugin-dev:skill-reviewer** on the rewritten SKILL.md.
- **fresh-lore** cross-checks SKILL.md against `.lore/work/brainstorm/distill-function.md` — did the rewrite preserve the brainstorm's shape rule and null-output-valid stance without reintroducing spec-replacement ambition?
...
**Expertise**: skill-prompt design. Both skills are capture skills under the capture-skill principles (`.lore/work/brainstorm/principles-for-capture-skills.md`). Prompt design is the deliverable; over-specifying reproduces the exact pathology the rewrite is meant to prevent.
...
- **fresh-lore** cross-checks against `.lore/work/brainstorm/principles-for-capture-skills.md` and `.lore/work/brainstorm/learn-dialog.md`. Did the rewrite honor all three principles?
...
1. **Cross-plugin coordination (Celeste)**:
   ...
   - Update the path reference `.lore/reference/vision.md` → `.lore/reference/vision.md`.
...
2. Update `lore-development/README.md`:
   - Rewrite the "`.lore/` structure" section (or equivalent) to describe the three-directory model: `work/` (work scaffolding, session-bound), `reference/` (solidified, system-oriented), `learned/` (mistakes-only, worker-oriented).
...
4. **Full spec AI Validation** (the custom checks from `.lore/work/specs/lore-redesign.md`):
...
- **REQ-REDESIGN-41 (`/learn` file layout default)**: spec defers internal layout to `.lore/work/issues/design-learned-structure.md`; plan adopts the spec's "starting default" (one file per entry, kebab-case, flat).
