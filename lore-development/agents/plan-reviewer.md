...
**Input**: Path to plan to review, or the agent will find the most recently modified plan in `.lore/build/plans/`
...
1. **Identify plan**: If path not specified, use Glob to find most recently modified file in `.lore/build/plans/`
...
8. **Stay in scope**: Review plans (`.lore/build/plans/`). Don't review specs (that's spec-reviewer's job) or designs (that's design-reviewer's job). If the plan's referenced spec has problems, note it briefly but don't do a full spec review.
